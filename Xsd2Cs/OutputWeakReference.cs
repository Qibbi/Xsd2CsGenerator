...
internal sealed class OutputWeakReference : AOutputType
{
    ...
    public OutputWeakReference(IEnumerable<List<XmlSchemaType>> schemaTypes, Settings settings, XmlSchemaType schemaType, Type? type) : base(settings, schemaType, type)
    {
        if (schemaType.UnhandledAttributes is not null)
        {
            foreach (XmlAttribute attribute in schemaType.UnhandledAttributes)
            {
                ...
                {
                    XmlRefType = GetSchemaType(schemaTypes, attribute.Value) ?? throw new InvalidDataException($"The ref type '{attribute.Value}' for TypedAssetId<{Name}> is not declared.");
                }
            }
        }
    }
    ...
    public override void Link(IReadOnlyList<OutputFile> files)
    {
        ...
        RefType = GetOutputType(files, XmlRefType) ?? throw new InvalidOperationException($"The ref type '{XmlRefType.Name}' for TypedAssetId<{Name}> could not be found.");
    }

    public override void WriteInheritedMemberDeclaration(int indent, StringBuilder sb)
    {
        WriteIndent(indent, sb).AppendLine($"public TypedAssetId<{RefType!.Name}> Value {{ get => _value; set => _value = value; }}");
    }
